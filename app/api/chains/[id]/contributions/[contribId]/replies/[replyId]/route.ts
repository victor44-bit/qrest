// app/api/chains/[id]/contributions/[contribId]/replies/[replyId]/route.ts
...
/* ---------------- GET: list children of a reply ---------------- */
export async function GET(req: NextRequest, { params }: { params: { id: string; contribId: string; replyId: string } }) {
  const chainId = String(params?.id ?? "");
  const replyId = String(params?.replyId ?? "");
  const take = parseTake(req.nextUrl.searchParams.get("take"), 10, 100);

  if (isBad(chainId)) return J(req, 400, { error: "bad-chainId" });
  if (isBad(replyId)) return J(req, 400, { error: "bad-parentId" });

  try {
    // Load parent reply and validate chain by reading parent's chainId
    const parentAgg = (await prisma.$runCommandRaw({
      aggregate: "Contribution",
      pipeline: [
        { $match: { $or: buildIdOr(replyId) } },
    ...
    // Fetch children under this reply
    ...
    console.error(`GET replies/[replyId] error:`, e);
    return J(req, 500, { error: `server-error: ${String(e?.message ?? e)}` });
  }
}

/* ---------------- POST: create child reply under replyId ---------------- */
export async function POST(req: NextRequest, { params }: { params: { id: string; contribId: string; replyId: string } }) {
  const chainId = String(params?.id ?? "");
  const parentId = String(params?.replyId ?? "");
  ...
  try {
    // Load parent reply and validate chain by parent's chainId
    const parentAgg = (await prisma.$runCommandRaw({
      aggregate: "Contribution",
      pipeline: [
    ...
    console.error(`POST replies/[replyId] error:`, e);
    return J(req, 500, { error: `server-error: ${String(e?.message ?? e)}` });
  }
}
